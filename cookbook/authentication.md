...
This guide demonstrates how to authenticate requests using Keychain tokens.
...
Route::middleware('auth:keychain')->group(function () {
...
```php
Route::middleware(['auth:keychain', 'abilities:users:read,users:write'])->group(function () {
...
```php
Route::middleware(['auth:keychain', 'ability:admin,moderator'])->group(function () {
...
```php
Route::middleware(['auth:keychain', 'token-type:sk'])->group(function () {
...
```php
Route::middleware(['auth:keychain', 'token-type:pk'])->group(function () {
...
```php
Route::middleware(['auth:keychain', 'token-type:sk,rk'])->group(function () {
...
```php
Route::middleware(['auth:keychain', 'environment:live'])->group(function () {
...
```php
Route::middleware(['auth:keychain', 'environment:test'])->group(function () {
...
class ApiTest extends TestCase
{
    public function test_user_can_access_api(): void
    {
        ...
        Keychain::actingAs($user, ['users:read', 'users:write']);
