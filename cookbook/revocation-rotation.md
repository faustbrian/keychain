...
Keychain::revoke($token->accessToken);
...
$group = Keychain::for($user)->issueGroup(['sk', 'pk', 'rk'], 'Payment Keys');
...
```php
Keychain::revoke($secretKey, RevocationMode::None);
...
Keychain::revoke($group->secretKey(), RevocationMode::Cascade);
...
```php
$group = Keychain::for($user)->issueGroup(['sk', 'pk', 'rk'], 'Keys');
Keychain::revoke($group->secretKey(), RevocationMode::Partial);
...
```php
$group = Keychain::for($user)->issueGroup(['sk', 'pk', 'rk'], 'Keys');
Keychain::revoke($group->secretKey(), RevocationMode::Timed);
...
$newToken = Keychain::rotate($token->accessToken);
...
```php
$newToken = Keychain::rotate($token->accessToken, RotationMode::Immediate);
...
```php
$newToken = Keychain::rotate($token->accessToken, RotationMode::GracePeriod);
...
```php
$newToken = Keychain::rotate($token->accessToken, RotationMode::DualValid);
...
$conductor = new TokenRevocationConductor(app(KeychainManager::class), $token->accessToken);
$conductor
...
```php
use Cline\Keychain\Conductors\TokenRotationConductor;

$conductor = new TokenRotationConductor(app(KeychainManager::class), $token->accessToken);
$newToken = $conductor
