...
```php
$unlimitedToken = Keychain::for($user)
    ->rateLimit(null)
...
```php
$restrictedToken = Keychain::for($user)
...
RateLimiter::for('keychain', function ($request) {
...
```php
// routes/api.php
Route::middleware(['auth:keychain', 'throttle:keychain'])->group(function () {
...
```php
use Cline\Keychain\Exceptions\RateLimitException;
...
```php
RateLimiter::for('keychain-endpoint', function ($request) {
    $token = $request->user()?->currentAccessToken();
