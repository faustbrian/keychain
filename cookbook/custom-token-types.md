...
```php
$webhookToken = Keychain::for($user)->issue('wh', 'Stripe Webhooks');
$tempToken = Keychain::for($user)->issue('tmp', 'One-time Access');
```
...
// config/keychain.php
'types' => [
    'wh' => ['class' => App\Keychain\WebhookTokenType::class],
],
```
