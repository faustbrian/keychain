...
Token derivation requires the `cline/lineage` package for hierarchical management:
...
Configure derivation in `config/keychain.php`:
...
$masterToken = Keychain::for($reseller)
...
```php
$customerToken = Keychain::derive($masterToken->accessToken)
...
$platform = Keychain::for($admin)
    ->issue('sk', 'Platform Master', abilities: ['*']);

// Level 1: Reseller
$reseller = Keychain::derive($platform->accessToken)
    ->abilities(['customers:manage', 'billing:read'])
    ->as('Reseller Key');

// Level 2: Customer
$customer = Keychain::derive($reseller->accessToken)
    ->abilities(['billing:read'])
    ->as('Customer Key');

// Check depth (0-indexed)
$depth = $customer->accessToken->getLineageDepth('token_derivation');
// 2
```
...
Keychain::revoke($masterToken->accessToken)->withDescendants();
...
$level3 = Keychain::derive($level2->accessToken)->abilities(['*'])->as('Level 3');
// Throws CannotDeriveTokenException
```
...
$resellerMaster = Keychain::for($reseller)
...
foreach ($reseller->customers as $customer) {
    $customerKey = Keychain::derive($resellerMaster->accessToken)
...
Keychain::revoke($resellerMaster->accessToken)->withDescendants();
```
