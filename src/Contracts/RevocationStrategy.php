 ...
 *         PersonalAccessToken::where('rotation_chain_id', $token->rotation_chain_id)
 *             ->update(['revoked_at' => now()]);
 *     }
 *
 *     public function getAffectedTokens(PersonalAccessToken $token): Collection
 *     {
 *         return PersonalAccessToken::where('rotation_chain_id', $token->rotation_chain_id)->get();
 *     }
 * }
 * ```
 *
...
interface RevocationStrategy
{
     ...
     *
     * @param PersonalAccessToken $token The token to revoke (may trigger cascade)
     */
    public function revoke(PersonalAccessToken $token): void;
     ...
     *
     * @param  PersonalAccessToken                  $token The token to check revocation impact for
     * @return Collection<int, PersonalAccessToken> All tokens that would be revoked
     */
    public function getAffectedTokens(PersonalAccessToken $token): Collection;
}
