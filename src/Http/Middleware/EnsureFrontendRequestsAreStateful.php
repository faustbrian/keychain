...
namespace Cline\Keychain\Http\Middleware;

use Cline\Keychain\Exceptions\InvalidStatefulDomainException;
use Closure;
...
final class EnsureFrontendRequestsAreStateful
{
     ...
     * configured stateful domains in the keychain.stateful config array.
     *
    ...
    public static function fromFrontend(Request $request): bool
    {
        ...
        $statefulConfig = config('keychain.stateful', []);
     ...
     * CSRF protection. Values are configurable via keychain.session config.
     */
    private function configureSecureCookieSessions(): void
    {
        config([
            'session.http_only' => config('keychain.session.http_only', true),
            'session.same_site' => config('keychain.session.same_site', 'lax'),
        ]);
    }
     ...
     * configured via the keychain.middleware config settings.
     *
    ...
    private function frontendMiddleware(): array
    {
        ...
        $configuredMiddleware = [
            config('keychain.middleware.encrypt_cookies', EncryptCookies::class),
            config('keychain.middleware.add_queued_cookies', AddQueuedCookiesToResponse::class),
            config('keychain.middleware.start_session', StartSession::class),
            config('keychain.middleware.validate_csrf_token', config('keychain.middleware.verify_csrf_token', VerifyCsrfToken::class)),
            config('keychain.middleware.authenticate_session'),
        ];
        ...
        array_unshift($middleware, function (Request $request, Closure $next): mixed {
            $request->attributes->set('keychain', true);
