 ...
 * );
 *
 * $adminToken = $user->createAccessToken(
 *     type: 'secret_key',
 *     name: 'Admin Access',
 *     abilities: ['*']
 * );
...
trait HasAccessTokens
{
    ...
    public static function cascadeDeleteTokens(): void
    {
        static::deleting(static function (Model $model): void {
            if ($model instanceof HasAccessTokensContract) {
                $model->accessTokens()->delete();
                $model->accessTokenGroups()->delete();
            }
        });
    }
     ...
     *
     * Inverse of accessTokenCan(), providing a more expressive API for checking
     ...
     */
    public function accessTokenCant(string $ability): bool
    {
        return !$this->accessTokenCan($ability);
    }
