...
use Cline\Keychain\Exceptions\InvalidTokenableException;
 ...
 * $resellerMaster = Keychain::for($reseller)->issue('sk', 'Reseller Master');
 *
 * // Derive customer token
 * $customerToken = Keychain::derive($resellerMaster->accessToken)
...
final readonly class TokenDerivationConductor
{
     ...
     *
     * @param KeychainManager        $manager         core keychain manager instance providing
     *                                                access to token generation, hashing, and
     *                                                audit functionality
     * @param PersonalAccessToken    $parentToken     The parent token from which the child token
    ...
    public function __construct(
        ...
        private PersonalAccessToken $parentToken,
     ...
     * the parent via the Lineage hierarchy system.
     *
    ...
    public function as(string $name): NewAccessToken
    {
        ...
        if (!PersonalAccessToken::areAbilitiesSubset($this->abilities, $this->parentToken->abilities)) {
        ...
        /** @var PersonalAccessToken $derivedToken */
        ...
        // Add to hierarchy using Lineage
        /** @var string $hierarchyType */
        $hierarchyType = Config::get('keychain.derivation.hierarchy_type', 'token_derivation');
        Lineage::addToHierarchy($derivedToken, $hierarchyType, $this->parentToken);

        // Audit log
        $this->manager->auditDriver()->log($derivedToken, AuditEvent::Derived, [
            'parent_token_id' => $this->parentToken->id,
            'depth' => $derivedToken->getLineageDepth($hierarchyType),
        ]);
