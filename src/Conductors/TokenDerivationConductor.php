...
final readonly class TokenDerivationConductor
{
    ...
    public function as(string $name): NewAccessToken
    {
        ...
        $plainTextAccessToken = $generator->generate(
            $tokenType->prefix(),
            $this->parentToken->environment,
        );

        // Create token with inherited restrictions
        $owner = $this->parentToken->owner;

        throw_if($owner === null, MissingTokenableException::forParentToken());
        ...
        $derivedToken = $owner->accessTokens()->create($tokenData);
        ...
        return new NewAccessToken($derivedToken, $plainTextAccessToken);
    }
}
