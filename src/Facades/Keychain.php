...
/**
 * Laravel facade for Keychain token authentication manager.
 *
 * Provides static access to the KeychainManager for managing personal access tokens,
 ...
 * ```php
 * // Issue a new token
 * $token = Keychain::for($user)->create('Mobile App', ['posts:read']);
 *
 * // Find and validate tokens
 * $accessToken = Keychain::findToken($token);
 *
 * // Revoke tokens
 * Keychain::revoke($accessToken);
 * ```
 *
...
final class Keychain extends Facade
{
     ...
     *
     * Returns the service container binding for the KeychainManager instance
     * that this facade proxies to.
     *
     * @return string The fully qualified class name of KeychainManager
     */
