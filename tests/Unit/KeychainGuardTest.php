...
function createRawToken(Authenticatable $user, array $attributes): PersonalAccessToken
{
    $defaults = [
        'type' => 'secret_key',
        'environment' => 'testing',
    ...
    return Model::unguarded(fn () => PersonalAccessToken::query()->create(array_merge($defaults, $attributes)));
}

describe('KeychainGuard', function (): void {
    beforeEach(function (): void {
        Config::set('auth.providers.users.model', User::class);
        Config::set('app.key', 'base64:'.base64_encode(random_bytes(32)));

        Route::middleware(['auth:keychain'])->get('/test-auth', fn () => response()->json([
            'user_id' => Auth::id(),
    ...
    describe('Happy Path', function (): void {
        ...
        test('validates bearer token and returns user with token attached', function (): void {
            ...
            expect($response->json('user_id'))->toBe($user->id);
            expect($response->json('token_type'))->toBe(PersonalAccessToken::class);
        ...
        test('checks multiple stateful guards in order', function (): void {
            Config::set('keychain.guard', ['web', 'admin']);
    ...
    describe('Sad Path', function (): void {
        ...
        test('throws TokenExpiredException when token exceeds expiration minutes', function (): void {
            Config::set('keychain.expiration', 60);
    ...
    describe('Edge Cases', function (): void {
        test('returns user without token wrapper when session user lacks HasApiTokens trait', function (): void {
            ...
            Route::middleware(['auth:keychain'])->get('/test-no-tokens', fn () => response()->json([
                'user_id' => Auth::id(),
        ...
        test('returns null when provider config model is not a string', function (): void {
            // Configure keychain guard with a provider
            Config::set('auth.guards.keychain.provider', 'users');
