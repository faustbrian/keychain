...
use Cline\Keychain\Exceptions\AuthenticationException;
use Cline\Keychain\Exceptions\InvalidEnvironmentException;
use Cline\Keychain\Exceptions\InvalidTokenTypeException;
use Cline\Keychain\Exceptions\MissingAbilityException;
use Cline\Keychain\Http\Middleware\CheckAbilities;
use Cline\Keychain\Http\Middleware\CheckEnvironment;
use Cline\Keychain\Http\Middleware\CheckForAnyAbility;
use Cline\Keychain\Http\Middleware\CheckTokenType;
use Cline\Keychain\Http\Middleware\EnsureFrontendRequestsAreStateful;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Config;
use Tests\Fixtures\User;

function createMiddlewareTestToken(User $user, string $type = 'sk', array $attributes = []): PersonalAccessToken
{
    static $counter = 0;
    ++$counter;
    ...
    describe('fromFrontend() static method', function (): void {
        describe('Happy Path', function (): void {
            test('returns true when referer matches configured stateful domain', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['localhost', 'app.example.com']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
            ...
            test('returns true when origin matches configured stateful domain', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['app.example.com']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
            ...
            test('returns true when domain with port matches', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['localhost:3000']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
        ...
        describe('Sad Path', function (): void {
            test('returns false when no referer or origin header present', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['localhost']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
        ...
        describe('Edge Cases', function (): void {
            test('strips https protocol from domain', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['secure.example.com']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
            ...
            test('prefers referer over origin when both present', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['referer.example.com']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
            ...
            test('uses origin when referer is not present', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['origin.example.com']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
                $request->headers->set('origin', 'https://origin.example.com');
            ...
            test('handles multiple stateful domains', function (): void {
                // Arrange
                Config::set('keychain.stateful', ['app.example.com', 'admin.example.com', 'localhost:3000']);
                $request = Request::create('/test', Symfony\Component\HttpFoundation\Request::METHOD_GET);
        ...
        describe('Happy Path', function (): void {
            ...
            test('passes request through when not from frontend', function (): void {
                ...
                $response = $middleware->handle(
                    ...
                    function ($req) use (&$nextCalled): string {
                        $nextCalled = true;
                        // Keychain attribute should NOT be set for non-frontend
                        expect($req->attributes->get('keychain'))->toBeNull();
