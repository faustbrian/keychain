...
describe('AccessToken Model', function (): void {
    describe('Happy Path', function (): void {
        test('findAccessToken() locates token by plain text hash when no prefix', function (): void {
            ...
            $plainToken = $newToken->plainTextAccessToken;

            // Act
            $found = Bearer::findAccessToken($plainToken);
        ...
        test('findAccessToken() locates token by prefixed token with id', function (): void {
            // Arrange
            $user = createUser();
            $newToken = Bearer::for($user)->issue('sk', 'Test Token');

            // Create id|plaintext format
            $prefixedToken = $newToken->accessToken->id.'|'.$newToken->plainTextAccessToken;

            // Act
            $found = Bearer::findAccessToken($prefixedToken);
    ...
    describe('Sad Path', function (): void {
        test('findAccessToken() returns null when prefixed token id does not exist', function (): void {
            // Arrange
            $user = createUser();
            $newToken = Bearer::for($user)->issue('sk', 'Test Token');
            ...
            $invalidToken = sprintf('%d|%s', $nonExistentId, $newToken->plainTextAccessToken);

            // Act
            $found = Bearer::findAccessToken($invalidToken);

            // Assert
            expect($found)->toBeNull();
        });

        test('findAccessToken() returns null when prefixed token verification fails', function (): void {
            // Arrange
            $user = createUser();
            $newToken = Bearer::for($user)->issue('sk', 'Test Token');
    ...
    describe('Edge Cases', function (): void {
        ...
        test('findAccessToken() handles plain token without pipe separator', function (): void {
            // Arrange
            $user = createUser();
            $newToken = Bearer::for($user)->issue('sk', 'Test Token');
