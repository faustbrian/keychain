...
describe('AccessTokenGroupFactory', function (): void {
    describe('Happy Path', function (): void {
        ...
        test('creates multiple AccessTokenGroups', function (): void {
            ...
            // Act
            $accessTokenGroups = AccessTokenGroup::factory()->count($count)->for($user, 'owner')->create();

            // Assert
            expect($accessTokenGroups)->toHaveCount($count);

            foreach ($accessTokenGroups as $tokenGroup) {
                expect($tokenGroup)->toBeInstanceOf(AccessTokenGroup::class);
                expect($tokenGroup->name)->toBeString();
                expect($tokenGroup->owner_id)->toBe($user->id);
            }
    ...
    describe('Relationships', function (): void {
        test('AccessTokenGroup can have multiple tokens', function (): void {
            ...
            $token2 = createAccessToken($user);
            $token1->update(['group_id' => $tokenGroup->id]);
            $token2->update(['group_id' => $tokenGroup->id]);

            // Assert
            expect($tokenGroup->accessTokens)->toHaveCount(2);
            expect($tokenGroup->accessTokens->pluck('id')->toArray())->toContain($token1->id);
            expect($tokenGroup->accessTokens->pluck('id')->toArray())->toContain($token2->id);
        });
