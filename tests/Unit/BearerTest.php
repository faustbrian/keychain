...
describe('Bearer', function (): void {
    describe('Happy Path', function (): void {
        test('useAccessTokenModel sets custom model class', function (): void {
            ...
            $originalModel = BearerConfig::$accessTokenModel;

            // Act
            Bearer::useAccessTokenModel($customModel);

            // Assert
            expect(BearerConfig::$accessTokenModel)->toBe($customModel);
            expect(Bearer::accessTokenModel())->toBe($customModel);
        })->defer(function () use (&$originalModel): void {
            BearerConfig::$accessTokenModel = $originalModel ?? AccessToken::class;
        });
    ...
    describe('Edge Cases', function (): void {
        test('useAccessTokenModel accepts fully qualified class names', function (): void {
            ...
            // Assert
            expect(BearerConfig::$accessTokenModel)->toBe($fqcn);
        })->defer(function () use (&$originalModel): void {
            BearerConfig::$accessTokenModel = $originalModel ?? AccessToken::class;
        });
        ...
        test('model setters can be called multiple times', function (): void {
            ...
            $firstResult = BearerConfig::$accessTokenModel;

            Bearer::useAccessTokenModel($secondModel);
            $secondResult = BearerConfig::$accessTokenModel;
        ...
        test('default model values can be restored', function (): void {
            ...
            expect(BearerConfig::$accessTokenModel)->toBe('App\\Models\\CustomAccessToken');
            ...
            // Assert
            expect(BearerConfig::$accessTokenModel)->toBe($defaultAccessTokenModel);
