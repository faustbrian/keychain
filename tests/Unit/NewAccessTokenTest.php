...
describe('NewAccessToken', function (): void {
    describe('Happy Path', function (): void {
        test('creates instance with accessToken and plainTextAccessToken properties', function (): void {
            // Arrange
            ...
            expect($newAccessToken->plainTextAccessToken)->toBe($plainText);
        });
        ...
        test('plainTextAccessToken property holds the plain text string', function (): void {
            // Arrange
            $personalAccessToken = new AccessToken([
            ...
            // Assert
            expect($newAccessToken->plainTextAccessToken)->toBeString();
            expect($newAccessToken->plainTextAccessToken)->toBe('pk_dev_123abc456def789ghi');
        });

        test('toArray returns array with accessToken and plainTextAccessToken keys', function (): void {
            // Arrange
            $personalAccessToken = new AccessToken([
                'type' => 'secret_key',
                'environment' => 'testing',
            ...
            expect($array)->toHaveKeys(['accessToken', 'plainTextAccessToken']);
            expect($array['accessToken'])->toBe($personalAccessToken);
            expect($array['plainTextAccessToken'])->toBe($plainText);
        });
        ...
        test('jsonSerialize returns same as toArray', function (): void {
            ...
            expect($jsonSerializeResult)->toEqual([
                ...
                'plainTextAccessToken' => $plainText,
            ]);
        });
    ...
    describe('Edge Cases', function (): void {
        ...
        test('handles long plain text tokens', function (): void {
            ...
            // Assert
            expect($newAccessToken->plainTextAccessToken)->toHaveLength(264); // sk_test_ (8) + 256
            expect($newAccessToken->plainTextAccessToken)->toBe($longPlainText);
        });
        ...
        test('readonly properties cannot be modified', function (): void {
            ...
            // Act & Assert
            try {
                $newAccessToken->plainTextAccessToken = 'modified';
