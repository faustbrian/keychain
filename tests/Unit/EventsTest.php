...
use Cline\Keychain\Events\TokenAuthenticationFailed;
use Cline\Keychain\Events\TokenCreated;
use Cline\Keychain\Events\TokenGroupCreated;
use Cline\Keychain\Events\TokenRevoked;
use Cline\Keychain\Events\TokenRotated;
...
describe('TokenCreated Event', function (): void {
    ...
    it('works with different token types', function (): void {
        ...
        // Assert
        expect($event->token)->toBeInstanceOf(PersonalAccessToken::class)
            ->and($event->tokenType)->toBe('publishable_key')
            ->and($event->environment)->toBe('production');
    });
});

describe('TokenGroupCreated Event', function (): void {
    it('can instantiate with required parameters', function (): void {
        // Arrange
        $user = createUser();
        $group = $user->tokenGroups()->create(['name' => 'Test Group']);

        $token1 = createToken($user, 'sk');
        $token2 = createToken($user, 'pk');
        ...
        // Act
        $event = new TokenGroupCreated(
            group: $group,
            tokens: $tokens,
        );

        // Assert
        expect($event)->toBeInstanceOf(TokenGroupCreated::class)
    ...
    it('has publicly accessible group property', function (): void {
        // Arrange
        $user = createUser();
        $group = $user->tokenGroups()->create(['name' => 'API Group']);

        $tokens = new Collection();

        // Act
        $event = new TokenGroupCreated($group, $tokens);

        // Assert
        expect($event->group)
            ->toBeInstanceOf(TokenGroup::class)
            ->toBe($group);
    });
    ...
    it('has publicly accessible tokens property', function (): void {
        // Arrange
        $user = createUser();
        $group = $user->tokenGroups()->create(['name' => 'Service Group']);
    ...
    it('works with empty token collection', function (): void {
        // Arrange
        $user = createUser();
        $group = $user->tokenGroups()->create(['name' => 'Empty Group']);
        ...
        // Assert
        expect($event->group)->toBeInstanceOf(TokenGroup::class)
    ...
    it('works with multiple tokens in collection', function (): void {
        // Arrange
        $user = createUser();
        $group = $user->tokenGroups()->create(['name' => 'Multi Token Group']);

        $token1 = createToken($user, 'sk');
        $token2 = createToken($user, 'pk');
        $token3 = createToken($user, 'rk');
...
describe('TokenRotated Event', function (): void {
    it('can instantiate with required parameters', function (): void {
        // Arrange
        $user = createUser();
        $oldToken = createToken($user, 'sk', ['name' => 'Old Token']);
        $newToken = createToken($user, 'sk', ['name' => 'New Token']);
    ...
    it('has publicly accessible oldToken property', function (): void {
        // Arrange
        $user = createUser();
        $oldToken = createToken($user, 'sk', ['name' => 'Old']);
        $newToken = createToken($user, 'sk', ['name' => 'New']);
