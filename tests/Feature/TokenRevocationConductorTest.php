...
describe('TokenRevocationConductor', function (): void {
    describe('Happy Path', function (): void {
        it('revokes single token using default mode', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($token)->revoke();
        ...
        it('revokes token using cascade mode shorthand', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($group->secretKey())->cascade()->revoke();
        ...
        it('revokes token using explicit cascade mode', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($group->secretKey())->using(RevocationMode::Cascade)->revoke();
        ...
        it('revokes token with reason captured in audit log', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($token)->withReason($reason)->revoke();
        ...
        it('revokes with cascade mode and reason', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($group->secretKey())
                ->cascade()
                ->withReason($reason)
                ->revoke();
        ...
        it('revokes using partial mode for same-type tokens only', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($group->secretKey())->using(RevocationMode::Partial)->revoke();
        ...
        it('revokes using timed mode', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($token)->using(RevocationMode::Timed)->revoke();
        ...
        it('returns new instance when using different modes', function (): void {
            ...
            // Act
            $conductor1 = resolve(BearerManager::class)->revoke($token);
    ...
    describe('Sad Path', function (): void {
        it('falls back to single revocation when cascade mode used on ungrouped token', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($token)->cascade()->revoke();
        ...
        it('falls back to single revocation when partial mode used on ungrouped token', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($token)->using(RevocationMode::Partial)->revoke();
    ...
    describe('Edge Cases', function (): void {
        it('handles null group relationship gracefully', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($token->fresh())->cascade()->revoke();
        ...
        it('preserves reason through multiple configuration calls', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($token)
                ->withReason($reason)
        ...
        it('cascades only within same group', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($group1->secretKey())->cascade()->revoke();
        ...
        it('partial mode revokes server-side tokens even when triggered from client-side token', function (): void {
            ...
            // Act
            resolve(BearerManager::class)->revoke($pkToken)->using(RevocationMode::Partial)->revoke();
        ...
        it('allows revoking already revoked token', function (): void {
            ...
            resolve(BearerManager::class)->revoke($token->fresh())->revoke();
    ...
    describe('Regression', function (): void {
        it('maintains immutability of conductor instances', function (): void {
            ...
            $user = createUser();
            $token = createAccessToken($user);
            $conductor = resolve(BearerManager::class)->revoke($token);
        ...
        it('cascade shorthand equivalent to using RevocationMode::Cascade', function (): void {
            ...
            resolve(BearerManager::class)->revoke($group2->secretKey())->using(RevocationMode::Cascade)->revoke();

            // Assert - Both should have same result
            expect($group1->fresh()->accessTokens->every->isRevoked())->toBeTrue();
            expect($group2->fresh()->accessTokens->every->isRevoked())->toBeTrue();
