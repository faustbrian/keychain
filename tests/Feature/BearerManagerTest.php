...
describe('BearerManager - Token Finding', function (): void {
    it('finds token by id|plaintext format with valid hash', function (): void {
        ...
        $tokenString = $result->accessToken->id.'|'.$result->plainTextAccessToken;

        // Find token using BearerManager directly
        $foundToken = Bearer::findAccessToken($tokenString);
    ...
    it('returns null when id|plaintext format has non-existent id', function (): void {
        ...
        $tokenString = '99999|'.$result->plainTextAccessToken;
    ...
    it('finds token without pipe separator using hash lookup', function (): void {
        ...
        $foundToken = Bearer::findAccessToken($result->plainTextAccessToken);
    ...
    it('handles multiple pipe characters in token correctly', function (): void {
        ...
        $tokenString = $result->accessToken->id.'|'.$result->plainTextAccessToken.'|extra|data';
...
describe('BearerManager - Revocation with Audit Failures', function (): void {
    it('silently continues when audit driver throws exception during revocation', function (): void {
        ...
        resolve(BearerManager::class)->registerAuditDriver('failing', $failingDriver);
    ...
    it('completes revocation even when audit driver fails', function (): void {
        ...
        resolve(BearerManager::class)->registerAuditDriver('throwing', $throwingDriver);
...
describe('BearerManager - Registration Methods', function (): void {
    it('registers custom token type', function (): void {
        ...
        resolve(BearerManager::class)->registerTokenType('custom', $customType);

        // Verify we can retrieve it
        $retrieved = resolve(BearerManager::class)->tokenType('custom');
    ...
    it('registers custom token generator', function (): void {
        ...
        resolve(BearerManager::class)->registerTokenGenerator('custom', $customGenerator);

        // Verify we can retrieve it
        $retrieved = resolve(BearerManager::class)->tokenGenerator('custom');
    ...
    it('registers custom token hasher', function (): void {
        ...
        resolve(BearerManager::class)->registerTokenHasher('custom', $customHasher);

        // Verify we can retrieve it
        $retrieved = resolve(BearerManager::class)->tokenHasher('custom');
    ...
    it('registers custom audit driver', function (): void {
        ...
        resolve(BearerManager::class)->registerAuditDriver('custom', $customDriver);

        // Verify we can retrieve it
        $retrieved = resolve(BearerManager::class)->auditDriver('custom');
    ...
    it('registers custom revocation strategy', function (): void {
        ...
        resolve(BearerManager::class)->registerRevocationStrategy('custom', $customStrategy);

        // Verify we can retrieve it
        $retrieved = resolve(BearerManager::class)->revocationStrategy('custom');
    ...
    it('registers custom rotation strategy', function (): void {
        ...
        resolve(BearerManager::class)->registerRotationStrategy('custom', $customStrategy);

        // Verify we can retrieve it
        $retrieved = resolve(BearerManager::class)->rotationStrategy('custom');
    ...
    it('allows multiple registrations of same type', function (): void {
        ...
        resolve(BearerManager::class)->registerTokenGenerator('gen1', $generator1);
        resolve(BearerManager::class)->registerTokenGenerator('gen2', $generator2);

        expect(resolve(BearerManager::class)->tokenGenerator('gen1'))->toBe($generator1);
        expect(resolve(BearerManager::class)->tokenGenerator('gen2'))->toBe($generator2);
    });
    ...
    it('can override default implementations via registration', function (): void {
        ...
        resolve(BearerManager::class)->registerTokenHasher('sha256', $customHasher);

        $retrieved = resolve(BearerManager::class)->tokenHasher('sha256');
    ...
    it('registered components persist across multiple calls', function (): void {
        ...
        resolve(BearerManager::class)->registerTokenType('persistent', $customType);

        // Retrieve multiple times
        $first = resolve(BearerManager::class)->tokenType('persistent');
        $second = resolve(BearerManager::class)->tokenType('persistent');
        $third = resolve(BearerManager::class)->tokenType('persistent');
...
describe('BearerManager - Integration with Registered Components', function (): void {
    it('uses registered token generator for token issuance', function (): void {
        ...
        resolve(BearerManager::class)->registerTokenGenerator('test_custom', $customGenerator);
        ...
        expect($result->plainTextAccessToken)->toContain($testMarker);
    });
    ...
    it('uses registered revocation strategy in revoke operation', function (): void {
        ...
        resolve(BearerManager::class)->registerRevocationStrategy('logging', $loggingStrategy);
    ...
    it('uses registered rotation strategy in rotate operation', function (): void {
        ...
        resolve(BearerManager::class)->registerRotationStrategy('logging', $loggingStrategy);
