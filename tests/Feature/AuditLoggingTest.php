...
describe('Audit Logging', function (): void {
    it('logs token creation event', function (): void {
        ...
        $token = Keychain::for($user)->issue('sk', 'API Key');

        $auditLog = TokenAuditLog::query()->where('token_id', $token->accessToken->id)
    ...
    it('stores audit log metadata', function (): void {
        ...
        $auditLog = TokenAuditLog::query()->where('token_id', $token->accessToken->id)->first();
    ...
    it('links audit log to token', function (): void {
        $user = createUser();
        $token = Keychain::for($user)->issue('sk', 'Test Key');
    ...
    it('filters audit logs by event type', function (): void {
        ...
        $createdLogs = TokenAuditLog::query()->where('event', AuditEvent::Created)->get();
    ...
    it('maintains audit trail through token lifecycle', function (): void {
        ...
        $initialAuditCount = TokenAuditLog::query()->where('token_id', $token->accessToken->id)->count();
    ...
    it('stores ip address in audit log', function (): void {
        ...
        $auditLog = TokenAuditLog::query()->create([
            'token_id' => $token->id,
            'event' => AuditEvent::Authenticated,
    ...
    it('queries audit logs by date range', function (): void {
        ...
        $logs = TokenAuditLog::query()->whereBetween('created_at', [$startDate, $endDate])->get();
    ...
    it('retrieves audit logs for specific token', function (): void {
        ...
        $token2Logs = TokenAuditLog::query()->where('token_id', $token2->id)->get();
    ...
    it('counts audit events by type', function (): void {
        ...
        TokenAuditLog::query()->create(['token_id' => $token->id, 'event' => AuditEvent::Authenticated]);
        TokenAuditLog::query()->create(['token_id' => $token->id, 'event' => AuditEvent::Authenticated]);
        TokenAuditLog::query()->create(['token_id' => $token->id, 'event' => AuditEvent::RateLimited]);

        $authCount = TokenAuditLog::query()->where('token_id', $token->id)
    ...
    it('orders audit logs chronologically', function (): void {
        ...
        $log1 = TokenAuditLog::query()->create([
            'token_id' => $token->id,
            'event' => AuditEvent::Authenticated,
            'created_at' => now()->subHours(2),
        ]);

        $log2 = TokenAuditLog::query()->create([
            'token_id' => $token->id,
            'event' => AuditEvent::RateLimited,
            'created_at' => now()->subHour(),
        ]);

        $logs = TokenAuditLog::query()
    ...
    it('retrieves latest audit log for token', function (): void {
        ...
        $latest = TokenAuditLog::query()->create([
            'token_id' => $token->id,
            'event' => AuditEvent::RateLimited,
            'created_at' => now(),
        ]);

        $latestLog = TokenAuditLog::query()
            ->where('token_id', $token->id)
            ->whereIn('event', [AuditEvent::Authenticated, AuditEvent::RateLimited])
            ->latest('created_at')
            ->first();
    ...
    it('supports all audit event types', function (): void {
        ...
        foreach ($events as $event) {
            $log = TokenAuditLog::query()->create([
                'token_id' => $token->id,
                'event' => $event,
            ]);
