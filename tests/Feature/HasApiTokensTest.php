...
describe('HasApiTokens Trait', function (): void {
    describe('Token Group Creation', function (): void {
        it('creates token group with multiple types', function (): void {
            ...
            // Act
            $group = $user->createTokenGroup(
                types: ['sk', 'pk', 'rk'],
                name: 'Test Group',
            );

            // Assert
            expect($group)->toBeInstanceOf(TokenGroup::class);
    ...
    describe('Token Groups Relationship', function (): void {
        ...
        it('retrieves all token groups for user', function (): void {
            // Arrange
            $user = createUser();
            $user->createTokenGroup(['sk', 'pk'], 'Group 1');
            $user->createTokenGroup(['sk', 'rk'], 'Group 2');
        ...
        it('filters token groups by name', function (): void {
            // Arrange
            $user = createUser();
            $user->createTokenGroup(['sk', 'pk'], 'Production');
            $user->createTokenGroup(['sk', 'rk'], 'Development');
    ...
    describe('Model Deletion Cascade', function (): void {
        ...
        it('deletes token groups when user is deleted', function (): void {
            // Arrange
            $user = createUser();
            $group1 = $user->createTokenGroup(['sk', 'pk'], 'Group 1');
            $group2 = $user->createTokenGroup(['sk', 'rk'], 'Group 2');
            ...
            foreach ($groupIds as $groupId) {
                expect(TokenGroup::query()->find($groupId))->toBeNull();
            }
        });
        ...
        it('cascade deletes both tokens and token groups', function (): void {
            ...
            $group = $user->createTokenGroup(['sk', 'pk'], 'Test Group');
            ...
            foreach ($groupTokenIds as $groupTokenId) {
                expect(PersonalAccessToken::query()->find($groupTokenId))->toBeNull();
            }
        });
        ...
        it('does not affect other users tokens on deletion', function (): void {
            ...
            // Assert
            expect(PersonalAccessToken::query()->find($token1->id))->toBeNull();
            expect(PersonalAccessToken::query()->find($token2->id))->not->toBeNull();
        });
    });

    describe('Token Checking Without Current Token', function (): void {
        it('tokenIs returns false when no PersonalAccessToken set', function (): void {
            // Arrange
            $user = createUser();
        ...
        it('tokenEnvironment returns null when no PersonalAccessToken set', function (): void {
            // Arrange
            $user = createUser();
        ...
        it('tokenType returns null when no PersonalAccessToken set', function (): void {
            // Arrange
            $user = createUser();
    ...
    describe('Edge Cases', function (): void {
        ...
        it('creates token group with null environment', function (): void {
            ...
            // Assert - when null is passed, Keychain uses its default environment
            expect($group->secretKey())->not->toBeNull();
            expect($group->publishableKey())->not->toBeNull();
        });
