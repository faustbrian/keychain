...
describe('TokenRotationConductor', function (): void {
    ...
    it('preserves abilities on rotation', function (): void {
        $user = createUser();
        $token = createAccessToken($user, 'sk', ['abilities' => ['users:read', 'posts:write']]);
        $manager = resolve(BearerManager::class);
    ...
    it('throws RuntimeException when owner is null', function (): void {
        $user = createUser();
        $token = createAccessToken($user);
        $manager = resolve(BearerManager::class);

        // Simulate a token with no owner by refreshing after deletion
        // This forces the owner relationship to return null
        $token->owner()->getRelated()::query()->delete();
        $token = $token->fresh();
        $token->setRelation('owner', null);

        $conductor = new TokenRotationConductor($manager, $token);

        expect(fn (): NewAccessToken => $conductor->immediate()->rotate())
            ->toThrow(RuntimeException::class, 'Token has no associated owner model');
    });
