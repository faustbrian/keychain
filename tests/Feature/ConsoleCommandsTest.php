...
describe('Prune Expired Command', function (): void {
    it('prunes expired tokens older than threshold', function (): void {
        ...
        $expiredToken = createAccessToken($user, 'pk', ['name' => 'Expired Token']);
        $expiredToken->update([
            'expires_at' => now()->subHours(25),
        ]);

        // Create recently expired token (within 24 hours)
        $recentlyExpiredToken = createAccessToken($user, 'pk', ['name' => 'Recently Expired']);
        ...
        // Assert
        expect($exitCode)->toBe(0);
        expect($user->accessTokens()->count())->toBe($initialCount - 1);
        expect($user->accessTokens()->find($expiredToken->id))->toBeNull();
        expect($user->accessTokens()->find($recentlyExpiredToken->id))->not->toBeNull();
        expect($user->accessTokens()->find($validToken->id))->not->toBeNull();
    });
    ...
    it('prunes revoked tokens older than threshold', function (): void {
        ...
        $recentlyRevokedToken = createAccessToken($user, 'sk', ['name' => 'Recently Revoked']);
        ...
        expect($exitCode)->toBe(0);
        expect($user->accessTokens()->count())->toBe($initialCount - 1);
        expect($user->accessTokens()->find($revokedToken->id))->toBeNull();
        expect($user->accessTokens()->find($recentlyRevokedToken->id))->not->toBeNull();
        expect($user->accessTokens()->find($validToken->id))->not->toBeNull();
    });
    ...
    it('respects --hours option override', function (): void {
        ...
        $recentToken = createAccessToken($user, 'pk', ['name' => 'Recent Token']);
        $recentToken->update([
            'expires_at' => now()->subHours(25),
        ]);
        ...
        expect($user->accessTokens()->count())->toBe($initialCount - 1);
        expect($user->accessTokens()->find($expiredToken->id))->toBeNull();
        expect($user->accessTokens()->find($recentToken->id))->not->toBeNull();
    });
    ...
    it('does not prune valid tokens', function (): void {
        ...
        // Create valid tokens
        $token1 = createAccessToken($user, 'sk', ['name' => 'Valid Token 1']);
        $token2 = createAccessToken($user, 'sk', ['name' => 'Valid Token 2']);
        $token3 = createAccessToken($user, 'pk', ['name' => 'Valid Token 3']);
        ...
        // Assert
        expect($exitCode)->toBe(0);
        expect($user->accessTokens()->count())->toBe($initialCount);
        expect($user->accessTokens()->find($token1->id))->not->toBeNull();
        expect($user->accessTokens()->find($token2->id))->not->toBeNull();
        expect($user->accessTokens()->find($token3->id))->not->toBeNull();
    });
    ...
    it('outputs correct info message', function (): void {
        ...
        $expiredToken1 = createAccessToken($user, 'pk', ['name' => 'Expired 1']);
        $expiredToken1->update(['expires_at' => now()->subHours(25)]);

        $expiredToken2 = createAccessToken($user, 'pk', ['name' => 'Expired 2']);
    ...
    it('prunes both expired and revoked tokens in single command', function (): void {
        ...
        // Assert
        expect($output)->toContain('Pruned 2 expired/revoked tokens');
        expect($user->accessTokens()->count())->toBe($initialCount - 2);
        expect($user->accessTokens()->find($expiredToken->id))->toBeNull();
        expect($user->accessTokens()->find($revokedToken->id))->toBeNull();
        expect($user->accessTokens()->find($validToken->id))->not->toBeNull();
    });
    ...
    it('prunes tokens from multiple users', function (): void {
        ...
        $expiredToken1 = createAccessToken($user1, 'pk', ['name' => 'User 1 Expired']);
        $expiredToken1->update(['expires_at' => now()->subHours(25)]);

        $expiredToken2 = createAccessToken($user2, 'pk', ['name' => 'User 2 Expired']);
        $expiredToken2->update(['expires_at' => now()->subHours(25)]);

        // Create valid tokens
        $validToken1 = createAccessToken($user1, 'sk', ['name' => 'User 1 Valid']);
        $validToken2 = createAccessToken($user2, 'sk', ['name' => 'User 2 Valid']);
        ...
        // Assert
        expect($output)->toContain('Pruned 2 expired/revoked tokens');
        expect($user1->accessTokens()->find($expiredToken1->id))->toBeNull();
        expect($user2->accessTokens()->find($expiredToken2->id))->toBeNull();
        expect($user1->accessTokens()->find($validToken1->id))->not->toBeNull();
        expect($user2->accessTokens()->find($validToken2->id))->not->toBeNull();
    });
});
