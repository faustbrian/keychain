...
describe('Token Rotation', function (): void {
    ...
    it('creates new token with same configuration', function (): void {
        ...
        expect($newToken->accessToken->tokenable_id)->toBe($oldToken->tokenable_id);
    });
    ...
    it('maintains group association on rotation', function (): void {
        $user = createUser();
        $group = Keychain::for($user)->issueGroup(['sk', 'pk'], 'Rotatable Keys');
        $oldToken = $group->secretKey();

        $newToken = Keychain::rotate($oldToken, 'immediate');

        expect($newToken->accessToken->group_id)->toBe($group->id);
        expect($group->fresh()->tokens)->toHaveCount(3);
    });
    ...
    it('rotates with grace period mode', function (): void {
        ...
        $newToken = Keychain::rotate($oldToken, 'grace_period');

        expect($newToken)->toBeInstanceOf(NewAccessToken::class);
        expect($newToken->plainTextToken)->not->toBe($oldToken->token);
    });
    ...
    it('rotates with dual valid mode', function (): void {
        ...
        $newToken = Keychain::rotate($oldToken, 'dual_valid');
    ...
    it('generates unique token on rotation', function (): void {
        ...
        $newToken = Keychain::rotate($token, 'immediate');
    ...
    it('rotates multiple times sequentially', function (): void {
        ...
        $token2 = Keychain::rotate($token1, 'immediate');
        $token3 = Keychain::rotate($token2->accessToken, 'immediate');
    ...
    it('increments user token count on rotation with dual valid', function (): void {
        ...
        Keychain::rotate($oldToken, 'dual_valid');

        expect($user->fresh()->tokens()->count())->toBe($initialCount + 2);
    });

    it('maintains same token count on rotation with immediate', function (): void {
        $user = createUser();
        $token = createToken($user);
        $countAfterCreate = $user->tokens()->count();

        Keychain::rotate($token, 'immediate');

        expect($user->fresh()->tokens()->count())->toBe($countAfterCreate + 1);
    });
});
