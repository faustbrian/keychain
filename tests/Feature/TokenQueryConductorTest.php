...
use Cline\Keychain\Conductors\TokenQueryConductor;
...
describe('TokenQueryConductor', function (): void {
    describe('type()', function (): void {
        it('filters tokens by type', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Secret Key']);
            createToken($user, 'pk', ['name' => 'Publishable Key']);
            createToken($user, 'rk', ['name' => 'Restricted Key']);
    ...
    describe('environment()', function (): void {
        it('filters tokens by environment', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['environment' => 'production']);
            createToken($user, 'sk', ['environment' => 'staging']);
            createToken($user, 'sk', ['environment' => 'development']);
    ...
    describe('valid()', function (): void {
        it('returns only non-expired and non-revoked tokens', function (): void {
            ...
            $expiredToken = Keychain::for($user)
        ...
        it('includes tokens with no expiration date', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Never Expires']);
    ...
    describe('expired()', function (): void {
        it('returns only expired tokens', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Valid Token']);
    ...
    describe('revoked()', function (): void {
        it('returns only revoked tokens', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Active Token']);

            $revokedToken1 = createToken($user, 'sk', ['name' => 'Revoked Token 1']);
            $revokedToken1->revoke();

            $revokedToken2 = createToken($user, 'sk', ['name' => 'Revoked Token 2']);
    ...
    describe('ungrouped()', function (): void {
        it('returns only tokens without a group', function (): void {
            ...
            createToken($user, 'sk', ['name' => 'Standalone Token 1']);
            createToken($user, 'pk', ['name' => 'Standalone Token 2']);

            Keychain::for($user)->issueGroup(['sk', 'pk'], 'Grouped Tokens');
        ...
        it('returns empty collection when all tokens are grouped', function (): void {
            // Arrange
            $user = createUser();
            Keychain::for($user)->issueGroup(['sk', 'pk'], 'Group 1');
            Keychain::for($user)->issueGroup(['rk'], 'Group 2');
    ...
    describe('withAbility()', function (): void {
        ...
        it('returns empty collection when no tokens match ability', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['abilities' => ['users:read']]);
    ...
    describe('orderByCreated()', function (): void {
        it('orders tokens by created_at descending by default', function (): void {
            ...
            $token1 = createToken($user, 'sk', ['name' => 'First']);
            Sleep::sleep(1);
            $token2 = createToken($user, 'sk', ['name' => 'Second']);
            Sleep::sleep(1);
            $token3 = createToken($user, 'sk', ['name' => 'Third']);
    ...
    describe('orderByLastUsed()', function (): void {
        it('orders tokens by last_used_at descending by default', function (): void {
            ...
            $token3 = createToken($user, 'sk', ['name' => 'Token 3']);
        ...
        it('handles tokens with null last_used_at', function (): void {
            ...
            $token1 = createToken($user, 'sk', ['name' => 'Never Used']);

            $token2 = createToken($user, 'sk', ['name' => 'Recently Used']);
    ...
    describe('limit()', function (): void {
        it('limits the number of results', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Token 1']);
            createToken($user, 'sk', ['name' => 'Token 2']);
            createToken($user, 'sk', ['name' => 'Token 3']);
            createToken($user, 'sk', ['name' => 'Token 4']);
            createToken($user, 'sk', ['name' => 'Token 5']);
        ...
        it('works with other filters', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Secret 1']);
            createToken($user, 'sk', ['name' => 'Secret 2']);
            createToken($user, 'sk', ['name' => 'Secret 3']);
            createToken($user, 'pk', ['name' => 'Publishable 1']);
    ...
    describe('get()', function (): void {
        it('returns collection of tokens', function (): void {
            ...
            expect($result)->toHaveCount(2);
            expect($result->first())->toBeInstanceOf(PersonalAccessToken::class);
        });
    ...
    describe('first()', function (): void {
        it('returns first token', function (): void {
            // Arrange
            $user = createUser();
            $token1 = createToken($user, 'sk', ['name' => 'First Token']);
            createToken($user, 'sk', ['name' => 'Second Token']);

            // Act
            $result = new TokenQueryConductor($user)->first();

            // Assert
            expect($result)->toBeInstanceOf(PersonalAccessToken::class);
        ...
        it('works with filters', function (): void {
            ...
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Secret Key']);
            $publishableToken = createToken($user, 'pk', ['name' => 'Publishable Key']);
    ...
    describe('count()', function (): void {
        ...
        it('works with filters', function (): void {
            ...
            createToken($user, 'sk', ['name' => 'Secret 1']);
            createToken($user, 'sk', ['name' => 'Secret 2']);
            createToken($user, 'pk', ['name' => 'Publishable']);
    ...
    describe('exists()', function (): void {
        it('returns true when tokens exist', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Test Token']);
    ...
    describe('toQuery()', function (): void {
        ...
        it('allows custom query modifications', function (): void {
            // Arrange
            $user = createUser();
            createToken($user, 'sk', ['name' => 'Alpha Token']);
            createToken($user, 'sk', ['name' => 'Beta Token']);
            createToken($user, 'sk', ['name' => 'Gamma Token']);
    ...
    describe('method chaining', function (): void {
        ...
        it('chains filter and ordering methods', function (): void {
            ...
            $token1 = createToken($user, 'sk', ['name' => 'Oldest']);
            Sleep::sleep(1);
            $token2 = createToken($user, 'sk', ['name' => 'Middle']);
            Sleep::sleep(1);
            $token3 = createToken($user, 'sk', ['name' => 'Newest']);

            createToken($user, 'pk', ['name' => 'Not Secret']);
        ...
        it('chains filter, ordering, and limit methods', function (): void {
            ...
            createToken($user, 'sk', ['name' => 'Valid 1']);
            createToken($user, 'sk', ['name' => 'Valid 2']);
            createToken($user, 'sk', ['name' => 'Valid 3']);

            $expired = Keychain::for($user)
                ->expiresAt(now()->subDay())
                ->issue('sk', 'Expired');
        ...
        it('chains group and ability filters', function (): void {
            ...
            $group = Keychain::for($user)
                ->abilities(['users:read'])
                ->issueGroup(['sk', 'pk'], 'Read Only Group');

            createToken($user, 'sk', ['abilities' => ['users:write']]);
    ...
    describe('edge cases', function (): void {
        ...
        it('handles user with only revoked tokens', function (): void {
            ...
            $token1 = createToken($user, 'sk', ['name' => 'Revoked 1']);
            $token1->revoke();

            $token2 = createToken($user, 'sk', ['name' => 'Revoked 2']);
            $token2->revoke();
        ...
        it('isolates tokens between different users', function (): void {
            ...
            createToken($user1, 'sk', ['name' => 'User 1 Token']);
            createToken($user2, 'sk', ['name' => 'User 2 Token']);
