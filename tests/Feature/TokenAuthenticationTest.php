...
describe('Token Authentication', function (): void {
    it('authenticates user with valid token', function (): void {
        $user = createUser();
        $newToken = Keychain::for($user)->issue('sk', 'API Key');

        $authenticatedToken = Keychain::findToken($newToken->plainTextToken);
    ...
    it('returns null for invalid token', function (): void {
        $result = Keychain::findToken('invalid_token_string');
    ...
    it('validates token by prefix', function (): void {
        ...
        $found = Keychain::findByPrefix($token->accessToken->prefix);
    ...
    it('uses actingAs for testing', function (): void {
        ...
        Keychain::actingAs($user, ['users:read', 'posts:write'], 'sk');
    ...
    it('checks token type on user', function (): void {
        $user = createUser();
        $token = Keychain::for($user)->issue('sk', 'Secret')->accessToken;
    ...
    it('finds token using findToken method', function (): void {
        $user = createUser();
        $newToken = Keychain::for($user)->issue('sk', 'Test');

        $found = Keychain::findToken($newToken->plainTextToken);
    ...
    it('creates multiple tokens for same user', function (): void {
        ...
        $token1 = Keychain::for($user)->issue('sk', 'Token 1');
        $token2 = Keychain::for($user)->issue('pk', 'Token 2');
        $token3 = Keychain::for($user)->issue('rk', 'Token 3');
    ...
    it('filters tokens by type', function (): void {
        ...
        Keychain::for($user)->issue('sk', 'Secret 1');
        Keychain::for($user)->issue('sk', 'Secret 2');
        Keychain::for($user)->issue('pk', 'Publishable');
    ...
    it('filters tokens by environment', function (): void {
        ...
        Keychain::for($user)->environment('production')->issue('sk', 'Prod 1');
        Keychain::for($user)->environment('production')->issue('sk', 'Prod 2');
        Keychain::for($user)->environment('development')->issue('sk', 'Dev');
