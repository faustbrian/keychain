...
use Cline\Keychain\Conductors\TokenDerivationConductor;
...
use Cline\Keychain\Exceptions\CannotDeriveTokenException;
use Cline\Keychain\Exceptions\InvalidDerivedAbilitiesException;
use Cline\Keychain\Exceptions\InvalidDerivedExpirationException;
...
describe('Token Derivation', function (): void {
    test('can derive token from parent', function (): void {
        ...
        $parentToken = Keychain::for($user)->issue('sk', 'Parent Token', abilities: ['users:read', 'users:write']);

        $childToken = Keychain::derive($parentToken->accessToken)
            ->abilities(['users:read'])
            ->as('Child Token');

        expect($childToken->accessToken)->toBeInstanceOf(PersonalAccessToken::class)
            ...
            ->and($childToken->plainTextToken)->toBeString();
    });

    test('derived token inherits parent restrictions', function (): void {
        $user = createUser();

        $parentToken = Keychain::for($user)
    ...
    test('can derive multiple children from one parent', function (): void {
        ...
        $parentToken = Keychain::for($user)->issue('sk', 'Parent Token', abilities: ['*']);

        $child1 = Keychain::derive($parentToken->accessToken)
            ->abilities(['users:read'])
            ->as('Child 1');

        $child2 = Keychain::derive($parentToken->accessToken)
    ...
    test('can derive nested tokens (grandchildren)', function (): void {
        ...
        $parentToken = Keychain::for($user)->issue('sk', 'Parent', abilities: ['users:read', 'posts:read']);

        $childToken = Keychain::derive($parentToken->accessToken)
            ->abilities(['users:read', 'posts:read'])
            ->as('Child');

        $grandchildToken = Keychain::derive($childToken->accessToken)
            ->abilities(['users:read'])
            ->as('Grandchild');
    ...
    test('throws exception when parent is revoked', function (): void {
        ...
        $parentToken = Keychain::for($user)->issue('sk', 'Parent', abilities: ['*']);
        $parentToken->accessToken->revoke();

        expect(fn (): mixed => Keychain::derive($parentToken->accessToken)
            ->abilities(['users:read'])
    ...
    test('throws exception when parent is expired', function (): void {
        ...
        $parentToken = Keychain::for($user)->expiresIn(1)->issue('sk', 'Parent', abilities: ['*']);
    ...
    test('throws exception when child abilities exceed parent abilities', function (): void {
        ...
        $parentToken = Keychain::for($user)->issue('sk', 'Parent', abilities: ['users:read']);
    ...
    test('throws exception when child expiration exceeds parent expiration', function (): void {
        ...
        $parentToken = Keychain::for($user)->expiresAt(now()->addDays(7))->issue('sk', 'Parent', abilities: ['*']);
    ...
    test('respects maximum derivation depth', function (): void {
        config(['keychain.derivation.max_depth' => 2]);

        $user = createUser();

        $level0 = Keychain::for($user)->issue('sk', 'Level 0', abilities: ['*']);
        $level1 = Keychain::derive($level0->accessToken)->abilities(['*'])->as('Level 1');
        $level2 = Keychain::derive($level1->accessToken)->abilities(['*'])->as('Level 2');

        expect(fn (): mixed => Keychain::derive($level2->accessToken)
            ->abilities(['*'])
    ...
    test('logs derived audit event', function (): void {
        ...
        assertDatabaseHas('token_audit_logs', [
    ...
    test('cascade descendants revocation strategy revokes all descendants', function (): void {
        ...
        $parent = Keychain::for($user)->issue('sk', 'Parent', abilities: ['*']);
        $child1 = Keychain::derive($parent->accessToken)->abilities(['*'])->as('Child 1');
        $child2 = Keychain::derive($parent->accessToken)->abilities(['*'])->as('Child 2');
        $grandchild = Keychain::derive($child1->accessToken)->abilities(['*'])->as('Grandchild');

        Keychain::revoke($parent->accessToken)->withDescendants();
    ...
    test('can check if token is root', function (): void {
        ...
        $parent = Keychain::for($user)->issue('sk', 'Parent', abilities: ['*']);
        $child = Keychain::derive($parent->accessToken)->abilities(['*'])->as('Child');
    ...
    test('can check if token can derive', function (): void {
        ...
        $validToken = Keychain::for($user)->issue('sk', 'Valid', abilities: ['*']);
        $revokedToken = Keychain::for($user)->issue('sk', 'Revoked', abilities: ['*']);
    ...
    test('parent and child use same environment', function (): void {
        ...
        $parentToken = Keychain::for($user)->environment('live')->issue('sk', 'Parent', abilities: ['*']);
    ...
    test('revoke API returns conductor', function (): void {
        ...
        $conductor = Keychain::revoke($parentToken->accessToken);
    ...
    test('derive API returns conductor', function (): void {
        ...
        $conductor = Keychain::derive($parentToken->accessToken);
