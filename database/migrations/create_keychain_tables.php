...
use Cline\Keychain\Enums\MorphType;
...
/**
 * Migration for creating Keychain token management database tables.
 *
 * This migration creates three tables for the Keychain API token management system:
 * - token_groups: stores token groups for organizational purposes
 * - personal_access_tokens: stores API token records with metadata and restrictions
 * - token_audit_logs: stores audit trail of token operations
 *
 * The primary key type (ID, ULID, UUID) and morph type (numeric, uuid, ulid, string)
 * are configured via the keychain.primary_key_type and keychain.morph_type
 * configuration options to support different application requirements.
 *
 * @see config/keychain.php
 */
return new class() extends Migration
{
    ...
    {
        $primaryKeyType = PrimaryKeyType::tryFrom(config('keychain.primary_key_type', 'id')) ?? PrimaryKeyType::Id;
        $morphType = MorphType::tryFrom(config('keychain.morph_type', 'string')) ?? MorphType::String;

        $connection = config('keychain.connection') ?? config('database.default');
        $useJsonb = DB::connection($connection)->getDriverName() === 'pgsql';

        $tableNames = config('keychain.table_names', [
            'personal_access_tokens' => 'personal_access_tokens',
            'token_groups' => 'token_groups',
            'token_audit_logs' => 'token_audit_logs',
        ]);

        // Create token_groups table first (referenced by tokens)
        Schema::create($tableNames['token_groups'], function (Blueprint $table) use ($primaryKeyType, $morphType, $useJsonb): void {
            ...
            // Indexes
            $table->index(['owner_type', 'owner_id'], 'token_groups_owner_idx');
        });

        // Create personal_access_tokens table
        Schema::create($tableNames['personal_access_tokens'], function (Blueprint $table) use ($primaryKeyType, $morphType, $tableNames, $useJsonb): void {
            // Primary key
            $this->addPrimaryKey($table, $primaryKeyType);

            // Polymorphic tokenable
            $this->addMorphColumns($table, 'tokenable', $morphType);

            // Group foreign key (nullable)
            $this->addForeignKey($table, 'group_id', $tableNames['token_groups'], $primaryKeyType, nullable: true);
            ...
            $table->index(['type', 'environment'], 'personal_access_tokens_type_env_idx');
            $table->index(['tokenable_type', 'tokenable_id'], 'personal_access_tokens_tokenable_idx');
            $table->index('prefix', 'personal_access_tokens_prefix_idx');
        });

        // Create token_audit_logs table
        Schema::create($tableNames['token_audit_logs'], function (Blueprint $table) use ($primaryKeyType, $tableNames, $useJsonb): void {
            $this->addPrimaryKey($table, $primaryKeyType);

            // Foreign key to token
            $this->addForeignKey($table, 'token_id', $tableNames['personal_access_tokens'], $primaryKeyType);
            ...
            // Indexes for common query patterns
            $table->index(['token_id', 'event'], 'token_audit_logs_token_event_idx');
            $table->index('created_at', 'token_audit_logs_created_idx');
        });
    }
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists($tableNames['token_audit_logs']);
        Schema::dropIfExists($tableNames['personal_access_tokens']);
        Schema::dropIfExists($tableNames['token_groups']);
    }
     ...
     *
     * @param Blueprint $table The table blueprint
     * @param string    $name  The morph column name prefix (e.g., 'tokenable')
