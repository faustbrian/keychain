...
### 1.1 Package Setup
- [ ] Copy skeleton-php to keychain directory
- [ ] Run configure.sh with: name=keychain, namespace=Cline\Keychain
...
│   └── Keychain.php
├── Guards/
│   └── KeychainGuard.php             # Auth guard implementation
├── Http/
│   └── Middleware/
│       ├── CheckAbilities.php
...
├── Keychain.php                      # Main manager class
├── KeychainManager.php               # Multi-guard manager
├── NewAccessToken.php                # DTO for newly created tokens
├── TransientToken.php                # In-memory token for testing
└── KeychainServiceProvider.php
```

## Phase 2: Core Models & Database

### 2.1 Migrations
- [ ] `create_keychain_tables` migration with:
...
- [ ] Configurable via `config('keychain.audit.driver')`
...
### 8.1 Configuration
- [ ] Per token type default: `config('keychain.types.sk.rate_limit')`
- [ ] Per environment: `config('keychain.environments.test.rate_limit')`
...
- [ ] Key format: `keychain:{token_id}` or `keychain:{type}:{env}:{tokenable}`
...
return [
    ...
    'morph_type' => env('KEYCHAIN_MORPH_TYPE', 'morph'),

    // Environments
    'environments' => [
        'allowed' => ['test', 'live'],
        'default' => env('KEYCHAIN_DEFAULT_ENV', 'test'),
        'rate_limits' => [
            'test' => 1000,
            'live' => 100,
        ],
...
- [ ] Register KeychainManager singleton
...
### 11.2 Facade
- [ ] `Keychain::for($user)->issue('sk')`
- [ ] `Keychain::for($user)->issueGroup(['sk', 'pk', 'rk'])`
- [ ] `Keychain::revoke($token)`
- [ ] `Keychain::rotate($token)`
- [ ] `Keychain::findByPrefix('sk_test_abc123')`
...
### 13.2 Test Helpers
- [ ] `Keychain::actingAs($user, ['abilities'], 'sk')`
